...
def parse_args():
    ...
    ap.add_argument(
        ...
        required=True,
        help=(
            "Input root. Supports either preprocessed/ or preprocessed/swimmercoseq/; "
            "the renderer loads swimmercoseq/fish1 + swimmercoseq/fish2 + swimmerbfnn/fish2."
        ),
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Y gap between swimmer lanes (auto-computed when omitted).",
    )
    ap.add_argument(
        "--y",
        dest="y_values",
        ...
        metavar=("Y1", "Y2", "Y3"),
        help=(
            "Manual Y lane centers for fish1 fish2 fish3 on the first selected frame. "
            "Overrides --y-gap."
        ),
    )
    ...
    CFG.fish_y_lines = tuple(args.y_values) if args.y_values is not None else None

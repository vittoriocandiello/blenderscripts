...
class Settings:
    ...
    # ── Camera (orthographic, top-down from +Z) ───────────────
    ortho_padding      = 1.10   # breathing room around both fish (lower = more zoom)
    camera_follow      = False  # False = fixed camera, True = follow fish trajectory
    camera_track_alpha = 1.00   # smoothing when camera_follow=True
    camera_height_min  = 8.0
    camera_height_mult = 2.3

    # ── Background ────────────────────────────────────────────
    # Pure black world. Illumination comes from scene lights.
    world_color      = (0.0, 0.0, 0.0)
    world_strength   = 0.0
    checkerboard_enable = True
    checker_tiles_x  = 22
    checker_margin   = 0.20
    checker_cover_scale = 1.30  # >1 grows checkerboard beyond camera footprint
    checker_color_a  = (0.035, 0.040, 0.050)
    checker_color_b  = (0.130, 0.140, 0.160)
    ...
    key_energy       = 520.0
    key_color        = (1.0, 0.96, 0.92)   # warm key
    key_size_factor  = 0.55

    fill_energy      = 260.0
    fill_color       = (0.86, 0.92, 1.0)   # cool fill
    fill_size_factor = 0.50

    rim_energy       = 240.0
    rim_color        = (0.95, 0.98, 1.0)
    ...
    # ── Fish layout ───────────────────────────────────────────
    fish_y_gap      = 0.0       # 0 = auto (1.3× Y extent)

    # ── Color themes ──────────────────────────────────────────
    # label → (R, G, B) in 0–1.
    #
    # Shared palette for both fish: clean, non-brown, publication-friendly.
    # The same label index has the same color on both fish.
    theme_fish1 = {
        "name": "Sapphire",
        "default": (0.22, 0.38, 0.52),     # body — sapphire slate
        0: (0.22, 0.38, 0.52),
        1: (0.92, 0.47, 0.48),             # coral
        2: (0.34, 0.78, 0.72),             # aqua mint
        3: (0.97, 0.77, 0.36),             # amber
        4: (0.44, 0.60, 0.92),             # periwinkle
        5: (0.67, 0.87, 0.46),             # lime
        6: (0.72, 0.53, 0.86),             # violet
        7: (0.97, 0.63, 0.39),             # apricot
        8: (0.45, 0.80, 0.58),             # sea green
        9: (0.91, 0.54, 0.74),             # rose
    }

    # Same palette as fish1 for direct visual comparison.
    theme_fish2 = {
        "name": "Sapphire",
        "default": (0.22, 0.38, 0.52),
        0: (0.22, 0.38, 0.52),
        1: (0.92, 0.47, 0.48),
        2: (0.34, 0.78, 0.72),
        3: (0.97, 0.77, 0.36),

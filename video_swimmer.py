#!/usr/bin/env python3
"""
video_swimmer.py — Headless Blender renderer for two swimmer streams.

Run:
    blender --background --python video_swimmer.py

All options are defined in `Settings` below.
"""

import os, sys, glob, math, tempfile, subprocess
import numpy as np
import bpy
...
class Settings:
    ...
    #input_dir       = "preprocessed/swimmerbfnn2"
    output_dir      = "renders/swimmers_video"
    frame_prefix    = "frame"
...
def render_video_frames(input_dir, output_dir):
    ...
    if len(frames1) != len(frames2):
        print(f"  Frame-count mismatch: fish1={len(frames1)} fish2={len(frames2)}; truncating to {n_total}")
    if n_total == 0:
        raise ValueError("No frames available to render.")

    selected = list(range(n_total))
    n_selected = len(selected)
    print(f"Rendering all {n_selected} frame(s): source index range 0 → {n_selected - 1}")
    ...
    for out_i, fi in enumerate(selected):
        path = os.path.join(output_dir, f"{CFG.frame_prefix}_{out_i:05d}.png")
        ...
        print(f"  {out_i + 1:5d}/{n_selected} src={fi:05d} → {os.path.basename(path)}")

    print(f"\n{n_selected} image(s) rendered to {output_dir}")
    return n_selected


def make_video(output_dir):
    video_path = os.path.join(output_dir, CFG.video_filename)
    cmd = [
        CFG.ffmpeg_bin,
        "-y",
        "-framerate", str(CFG.fps),
        "-i", os.path.join(output_dir, f"{CFG.frame_prefix}_%05d.png"),
        "-c:v", CFG.video_codec,
    ...
    print(f"\nStitching video → {video_path}")
    try:
        result = subprocess.run(cmd, capture_output=True, text=True)
    except FileNotFoundError:
        print(f"ffmpeg not found ('{CFG.ffmpeg_bin}'). PNG frames are still available.")
        return None
...
if __name__ == "__main__":
    try:
        ...
        print("=" * 50)
        n = render_video_frames(CFG.input_dir, CFG.output_dir)
        if n > 0:
            make_video(CFG.output_dir)
        print("\nDone!")
    except Exception as exc:
